...
def find_match(text, num_upper):
    upper_pattern = '[A-Z]{' + str(num_upper) + '}'
    pattern = upper_pattern + '[a-z]' + upper_pattern
    matches = re.findall('[^A-Z](' + pattern + ')[^A-Z]', text)
    return matches

def extract_middle(arr):
    mid = len(arr[0])//2
    return [word[mid] for word in arr]
...
-->
"""
test = 'OOOmOOOxOOOfOOO'
found = find_match(test, 3)
print(found, ''.join(extract_middle(found)), sep='\n')
